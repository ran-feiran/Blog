...
<!--              <el-checkbox label="weibo">微博</el-checkbox>-->
      ...
      <el-tab-pane label="社交信息" name="notice">
        <el-form label-width="70px" :model="websiteConfigForm"  v-loading="loading">
...
export default {
  ...
  methods: {
    getWebsiteConfig() {
      this.axios.get("/api/website/getWebsiteConfig").then(({ data }) => {
    ...
    updateWebsiteConfig() {
      this.axios
          .put("/api/website/updateWebsiteConfig", this.websiteConfigForm)
          .then(({ data }) => {
            if (data.flag) {
              this.$notify.success({
                title: "成功",
