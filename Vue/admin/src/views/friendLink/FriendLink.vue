...
export default {
  ...
  methods: {
    ...
    deleteLink(id) {
      ...
      this.axios.delete("/api/friendLink/del/batch", param).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: "成功",
    ...
    addOrEditCategory() {
      ...
      this.axios.post("/api/friendLink/addOrEditFriendLink", this.linkForm).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: "成功",
            message: data.message
