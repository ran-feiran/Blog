      ...
      <el-table-column label="操作" align="center" width="200">
        <template slot-scope="scope">
          <el-button
            ...
            v-if="scope.row.children && !scope.row.parentId"
          >
    ...
    <el-dialog :visible.sync="addResource" width="30%">
      ...
      <el-form label-width="80px" size="medium" :model="resourceForm">
        ...
        <el-form-item label="匿名访问">
          <el-switch
              v-model="resourceForm.isAnonymous"
              active-color="#13ce66"
              inactive-color="#F4F4F5"
              :active-value="1"
              :inactive-value="0"
...
export default {
  ...
  methods: {
    listResources() {
      this.axios
        .get("/api/resource/getResourcesInfoBack", {
          params: {
    ...
    changeResource(resource) {
        this.axios.put("/api/resource/updateAnonymousById", {
          id: resource.id,
    ...
    deleteResource(id) {
      this.axios.delete("/api/resource/del/" + id).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: "成功",
            message: data.message
    ...
    addOrEditResource() {
      if (this.resourceForm.resourceName == undefined) {
        this.$message.error("资源名不能为空");
        return false;
      }
      this.axios
        .post("/api/resource/addOrEditResource", this.resourceForm)
        .then(({ data }) => {
          if (data.flag) {
            this.$notify.success({
              title: "成功",
