      ...
      <el-table-column label="操作" align="center" width="150">
        <template slot-scope="scope">
          ...
          <el-popconfirm
            ...
            @confirm="updateArticleDelete(scope.row.articleId)"
            v-if="scope.row.isDelete == 0"
          >
          ...
          <el-popconfirm
            ...
            v-if="scope.row.isDelete == 1"
            title="确定彻底删除吗？"
            @confirm="deleteArticles(scope.row.articleId)"
          >
            <el-button size="mini" type="danger" slot="reference">
              删除
            </el-button>
...
export default {
  ...
  methods: {
    ...
    updateArticleDelete(id) {
      ...
      this.axios.put("/api/article/del/batch", param).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: "成功",
            message: data.message
    ...
    deleteArticles(id) {
      ...
      this.axios.delete("/api/article/del/batch", param).then(({ data }) => {
        if (data.flag) {
          this.$notify.success({
            title: "成功",
            message: data.message
    ...
    listCategories() {
      this.axios.get("/api/category/getCategoryList").then(({ data }) => {
        this.categoryList = data.data;
      });
    },
    listTags() {
      this.axios.get("/api/tag/getTagList").then(({ data }) => {
