...
<script>
  import {listLinks, addOrEditCategory} from '@/api/friendLink'
  export default {
    created() {
      this.listLinks();
    },
    ...
    methods: {
      ...
      async listLinks() {
        const res = await listLinks(this.current, this.size, this.keywords)
        const cons = res.data
        // console.log(cons)
        if (cons.flag) {
          this.linkList = cons.data.linkList;
      ...
      async addOrEditCategory() {
        ...
        const res = await addOrEditCategory(this.linkForm)
        const cons = res.data
        // console.log(cons)
        if (cons.flag) {
          this.$notify.success({
      ...
      deleteLink(id) {
        ...
        this.axios.delete("/api/links", param).then(({ data }) => {
          if (data.flag) {
            this.$notify.success({
              title: "成功",
