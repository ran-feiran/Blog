        ...
        <div class="article-info">
          <div class="first-line">
            ...
            <span>
              ...
              </svg> 发表于 {{ article.createTime | date }}
            </span>
          ...
          <div class="second-line">
            ...
            <span>
              ...
              </svg> 字数统计: {{ wordNum | num }}
            </span>
            <span class="separator">|</span>
            <!-- 阅读时长 -->
            <span>
              <svg class="icon" aria-hidden="true" style="font-size: 16px;">
                <use xlink:href="#icon-yuedumoshi"></use>
              </svg> 阅读时长: {{ readTime }}
            </span>
          </div>
          <div class="third-line">
            ...
            <span>
              <svg class="icon" aria-hidden="true" style="font-size: 15px">
                <use xlink:href="#icon-yanjingliulankeshi"></use>
              </svg> 阅读量: {{ article.viewsCount }}
            </span>
            <span class="separator">|</span>
            <!-- 评论量 -->
            <span>
      ...
      <v-col md="9" cols="12">
        <v-card class="article-wrapper">
          ...
          <div class="aritcle-copyright">
            <div>
              <span>文章作者：</span>
              <a href="/" target="_blank"> {{ article.nickname }}</a>
            </div>
            <div>
              <span>文章链接：</span>
          ...
          <div class="article-reward">
            ...
            <a :class="isLike" @click="like">
              ...
              <span v-show="article.likeCount > 0">{{ article.likeCount | num }}</span>
            </a>
...
export default {
  ...
  methods: {
    getArticle() {
      ...
      //查询文章
      this.axios.get("/api/article/blog/" + arr[2]).then((res) => {
        const cons = res.data;
        document.title = cons.data.articleTitle;
        this.article = cons.data;
        //将markdown转换为Html
    ...
    // 图片预览
    previewImg(img) {
      // console.log(this.imgList.indexOf(img))
      this.$imagePreview({
        images: this.imgList,
    ...
    like() {
      ...
      //发送请求
      this.axios.post("/api/article/like/"+this.article.articleId).then((res) => {
        const cons = res.data;
        if (cons.flag) {
          //判断是否点赞
          if (this.$store.state.articleLikeSet.indexOf(this.article.articleId) != -1) {
  ...
  computed: {
    ...
    isLike() {
      ...
      return articleLikeSet.indexOf(this.article.articleId) != -1 ? "like-btn-active" : "like-btn";
    },
    isFull() {
      return function(articleId) {
        return articleId ? "post full" : "post";
      };
    }
  }
};
